function [SPMs]=PatternAnalysis_SingleTrial_fMRI_design(subject,phase,pattern,roi)
%new = PatternAnalysis_SingleTrial_fMRI_design(2,2,'^Realigned.*nii$',38);
%
% Generates SPM structure array for Mumfordian single trial analysis
% by-passing the SPM interface for a given subject and phase. It will be
% ready to be ran for a given PATTERN and ROI.
%
% Example Usage: [SPMs]=PatternAnalysis_SingleTrial_fMRI_design(2,4,'^Realigned.*nii',44)
...
load(GetSubjectData(subject,phase(end),'stimulation'));
csp               = p.stim.cs_plus;
...
for t = trials;%current trial
    ...
    SPM.external_trial_id = conds(t); 
    SPM.external_onsets   = onsets(t);
    ...
    for i = 1:2;%two regressors (first one with one trial, and the other one with all the rest)
        Sess.U(i).dt            = xBF.dt;%- time bin (seconds)
        if i == 1%if we are in the first regressor simply put the current trials' onset
            Sess.U(i).ons       = onsets(t);%- onsets    (in SPM.xBF.UNITS)
            Sess.U(i).name      = {sprintf('%02d-%03d',conds(t),t)};%- cell of names for each input or cause
        else%otherwise all the rest.
            Sess.U(i).ons       = setdiff(onsets,onsets(t));%- onsets    (in SPM.xBF.UNITS)
            Sess.U(i).name      = {sprintf('All minus %02d-%03d',conds(t),t)};%- cell of names for each input or cause
        end
    ...
    %% append all to the output variable
    SPMs(t) = SPM;
end
